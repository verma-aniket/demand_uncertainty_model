...
proj_output_dir = REPO_ROOT / "Results" / "Projections" / "City"
comp_output_dir = REPO_ROOT / "Results" / "Components" / "City"
...
pop_mat = np.loadtxt(scenario_dir / 'population_scenarios.csv', delimiter=",").T
...
price_df = pd.read_csv(input_dir / 'tier_prices_2021.csv')
tier_price_dict = {}
...
hh_pool_mat = pd.read_csv(input_dir / 'total_housing_pool.csv', usecols=hh_char_terms).to_numpy()            # smallest useful form
hh_pool_pipe_sizes = pd.read_csv(input_dir / 'total_housing_pool.csv', usecols=['pipe_size']).to_numpy()     # for price sampling
...
hh_pool_ini = to_row_lists(pd.read_csv(input_dir / '2021_housing_pool.csv', usecols=hh_char_terms).to_numpy())   # initial housing stock
price_mat_ini = to_row_lists(get_tier_prices(pipe_sizes=pd.read_csv(input_dir / '2021_housing_pool.csv', usecols=['pipe_size']).to_numpy(),
...
id_grid = np.load(input_dir / f"ID_Grid/input_id_chunk_{array_id}.npy")
...
# Save Results
np.save(proj_output_dir /  f"proj_chunk_{array_id}.npy", demand_matrix)
if store_components: 
    np.save(comp_output_dir / f"comp_chunk_{array_id}.npy", components_matrix)
